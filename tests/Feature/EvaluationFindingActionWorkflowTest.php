...
class EvaluationFindingActionWorkflowTest extends TestCase
{
    ...
    public function test_weighted_multi_owner_action_plan_reaches_independently_verified_closure(): void
    {
        ...
        $this->actingAs($coordinator)->post(route('monitoring-evaluation.findings.actions.store', [$coordinator->currentTeam->slug, $finding]), $this->actionPayload($firstOwner, 'ACT-01', 40))->assertRedirect();
        $this->actingAs($coordinator)->post(route('monitoring-evaluation.findings.actions.store', [$coordinator->currentTeam->slug, $finding]), $this->actionPayload($secondOwner, 'ACT-02', 60))->assertRedirect();
        ...
        $this->actingAs($closer)->patch(route('monitoring-evaluation.findings.close', [$closer->currentTeam->slug, $finding]), ['note' => 'Premature closure.'])->assertStatus(409);
        ...
        $this->actingAs($closer)->patch(route('monitoring-evaluation.findings.close', [$closer->currentTeam->slug, $finding]), ['note' => 'All weighted actions have independent completion evidence.'])->assertRedirect();
        ...
        $this->actingAs($coordinator)->get(route('evidence.preview', [$coordinator->currentTeam->slug, $actionDocument]))->assertOk();
        $outsider = User::factory()->countyAdmin(County::factory()->create())->create();
        $this->actingAs($outsider)->get(route('evidence.preview', [$outsider->currentTeam->slug, $actionDocument]))->assertForbidden();
        $this->actingAs($coordinator)->get(route('monitoring-evaluation.index', $coordinator->currentTeam->slug))->assertOk()->assertInertia(fn (Assert $page) => $page
    ...
    public function test_action_plan_rejects_over_allocation_cross_county_owner_and_unlinked_evidence(): void
    {
        ...
        $this->actingAs($coordinator)->post(route('monitoring-evaluation.findings.actions.store', [$coordinator->currentTeam->slug, $finding]), $this->actionPayload($owner, 'ACT-01', 70))->assertRedirect();
        $this->actingAs($coordinator)->post(route('monitoring-evaluation.findings.actions.store', [$coordinator->currentTeam->slug, $finding]), $this->actionPayload($owner, 'ACT-02', 40))->assertSessionHasErrors('weight_percentage');
        $this->actingAs($coordinator)->post(route('monitoring-evaluation.findings.actions.store', [$coordinator->currentTeam->slug, $finding]), $this->actionPayload($otherOwner, 'ACT-03', 30))->assertStatus(422);
        ...
        $this->actingAs($owner)->post(route('monitoring-evaluation.finding-actions.updates.store', [$owner->currentTeam->slug, $action]), ['assessment_document_id' => $unlinked->id, 'progress_percentage' => 50, 'narrative' => 'Unlinked evidence.'])->assertStatus(409);

        $closer = User::factory()->devolutionAdmin()->create();
        $this->actingAs($closer)->patch(route('monitoring-evaluation.findings.close', [$closer->currentTeam->slug, $finding]), ['note' => 'Underweighted plan.'])->assertStatus(409);
    }
    ...
    private function submitAndVerify(EvaluationFindingAction $action, User $owner, User $verifier, int $progress, string $sourceType): void
    {
        ...
        $this->actingAs($owner)->post(route('monitoring-evaluation.finding-actions.documents.store', [$owner->currentTeam->slug, $action]), $upload)->assertRedirect();
        $document = $action->documentLinks()->latest()->firstOrFail()->document;
        $this->actingAs($owner)->post(route('monitoring-evaluation.finding-actions.updates.store', [$owner->currentTeam->slug, $action]), ['assessment_document_id' => $document->id, 'progress_percentage' => $progress, 'narrative' => "Implementation progress reached {$progress} percent."])->assertRedirect();
        $update = EvaluationFindingActionUpdate::query()->where('status', 'pending_verification')->sole();
        $this->actingAs($owner)->patch(route('monitoring-evaluation.finding-action-updates.verify', [$owner->currentTeam->slug, $update]), ['decision' => 'verified', 'note' => 'Self verification.'])->assertForbidden();
        $this->actingAs($verifier)->patch(route('monitoring-evaluation.finding-action-updates.verify', [$verifier->currentTeam->slug, $update]), ['decision' => 'verified', 'note' => 'Evidence confirms the reported progress.'])->assertRedirect();
