...
class IntegrationManagementTest extends TestCase
{
    ...
    public function test_contract_is_independently_published_and_sandbox_exchange_is_validated_encrypted_idempotent_and_exported(): void
    {
        ...
        $this->actingAs($author)->post(route('integrations.systems.store', $author->currentTeam->slug), $this->systemPayload(['owner_organization_id' => $organization->id]))->assertRedirect();
        ...
        $this->actingAs($author)->get(route('integrations.index', $author->currentTeam->slug))->assertOk()->assertInertia(fn (Assert $page) => $page
        ...
        foreach (['csv', 'xlsx', 'json', 'pdf'] as $format) {
            $this->actingAs($author)->get(route('workspace.export', [$author->currentTeam->slug, 'integration-systems', $format]))->assertOk()->assertDownload();
        }
        $systemCsv = $this->actingAs($author)->get(route('workspace.export', [$author->currentTeam->slug, 'integration-systems', 'csv']))->streamedContent();
        $this->assertStringContainsString('Reference release', $systemCsv);
        $this->assertStringContainsString($release->checksum, $systemCsv);
        $this->actingAs($author)->post(route('integrations.contracts.store', $author->currentTeam->slug), $this->contractPayload($system))->assertRedirect();
        ...
        $this->actingAs($author)->patch(route('integrations.contracts.publish', [$author->currentTeam->slug, $contract]), ['effective_from' => now()->subMinute()->toIso8601String()])->assertForbidden();
        $this->actingAs($reviewer)->patch(route('integrations.contracts.publish', [$reviewer->currentTeam->slug, $contract]), ['effective_from' => now()->subMinute()->toIso8601String()])->assertRedirect();
        ...
        $this->actingAs($author)->post(route('integrations.exchanges.dispatch', [$author->currentTeam->slug, $contract]), $request)->assertRedirect();
        ...
        $this->actingAs($author)->post(route('integrations.exchanges.dispatch', [$author->currentTeam->slug, $contract]), [...$request, 'idempotency_key' => 'IPPD-INVALID', 'payload' => ['employee_reference' => 'IPPD-000124']])->assertSessionHasErrors('payload.full_name');
        $this->assertDatabaseCount('integration_exchanges', 1);

        foreach (['csv', 'xlsx', 'json', 'pdf'] as $format) {
            $this->actingAs($author)->get(route('workspace.export', [$author->currentTeam->slug, 'integrations', $format]))->assertOk()->assertDownload();
        }
    ...
    public function test_production_transport_remains_closed_without_source_owner_activation_and_vault_credential(): void
    {
        ...
        $this->actingAs($author)->post(route('integrations.systems.store', $author->currentTeam->slug), $systemData)->assertRedirect();
        $system = IntegrationSystem::query()->sole();
        $this->actingAs($author)->post(route('integrations.contracts.store', $author->currentTeam->slug), $this->contractPayload($system))->assertRedirect();
        $contract = IntegrationContract::query()->sole();
        $this->actingAs($reviewer)->patch(route('integrations.contracts.publish', [$reviewer->currentTeam->slug, $contract]), ['source_owner_approval_reference' => 'TREASURY-CONTRACT-TEST-001', 'data_sharing_agreement_reference' => 'DSA-TEST-001', 'effective_from' => now()->subMinute()->toIso8601String()])->assertRedirect();
        $this->actingAs($author)->post(route('integrations.exchanges.dispatch', [$author->currentTeam->slug, $contract]), ['idempotency_key' => 'IFMIS-CLOSED-GATE-001', 'payload' => ['employee_reference' => 'IPPD-000125', 'full_name' => 'Test User', 'employment_status' => 'active']])->assertRedirect();
        $exchange = IntegrationExchange::query()->sole();
        ...
        $this->actingAs($author)->patch(route('integrations.systems.activate', [$author->currentTeam->slug, $system]), ['production_approval_reference' => 'TREASURY-ACTIVATION-001', 'production_approved_at' => now()->subMinute()->toIso8601String()])->assertForbidden();
        $this->actingAs($reviewer)->patch(route('integrations.systems.activate', [$reviewer->currentTeam->slug, $system]), ['production_approval_reference' => 'TREASURY-ACTIVATION-001', 'production_approved_at' => now()->subMinute()->toIso8601String()])->assertRedirect();
        ...
        $this->actingAs($author)->post(route('integrations.exchanges.dispatch', [$author->currentTeam->slug, $contract]), ['idempotency_key' => 'IFMIS-ACTIVE-001', 'payload' => ['employee_reference' => 'IPPD-000126', 'full_name' => 'Test User Two', 'employment_status' => 'active']])->assertRedirect();
    ...
    public function test_system_registration_fails_closed_for_missing_corrupt_or_incomplete_catalogue_state(): void
    {
        ...
        $this->actingAs($author)->post(route('integrations.systems.store', $author->currentTeam->slug), $payload)->assertStatus(409);
    ...
    public function test_reconciliation_exception_resolution_is_county_scoped_and_closes_run_with_checksum(): void
    {
        ...
        $this->actingAs($outsider)->patch(route('integrations.exceptions.resolve', [$outsider->currentTeam->slug, $exception]), ['resolution' => 'Out-of-scope attempt.'])->assertForbidden();
        $this->actingAs($manager)->patch(route('integrations.exceptions.resolve', [$manager->currentTeam->slug, $exception]), ['resolution' => 'Identity steward verified the departure and disabled the unmatched local access record.'])->assertRedirect();
    ...
    public function test_partner_contribution_source_matching_is_idempotent_immutable_scoped_and_does_not_bypass_human_reconciliation(): void
    {
        ...
        foreach (['csv', 'json'] as $format) {
            $countyExport = $this->actingAs($countyViewer)->get(route('workspace.export', [$countyViewer->currentTeam->slug, 'integrations', $format]))->assertOk()->streamedContent();
            $this->assertStringContainsString('SRC-MATCHED-001', $countyExport);
            $this->assertStringContainsString('value_mismatch', $countyExport);
        }
        $this->actingAs($countyViewer)->get(route('workspace.export', [$countyViewer->currentTeam->slug, 'integrations', 'xlsx']))->assertOk()->assertDownload();
        $this->actingAs($countyViewer)->get(route('workspace.export', [$countyViewer->currentTeam->slug, 'integrations', 'pdf']))->assertOk()->assertHeader('content-type', 'application/pdf');
        $outsideExport = $this->actingAs($outsideViewer)->get(route('workspace.export', [$outsideViewer->currentTeam->slug, 'integrations', 'json']))->assertOk()->streamedContent();
