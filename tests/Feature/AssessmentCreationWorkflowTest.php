...
class AssessmentCreationWorkflowTest extends TestCase
{
    ...
    public function test_authorized_administrator_can_initiate_a_governed_assessment_with_complete_lineage(): void
    {
        ...
        $this->actingAs($administrator)->get(route('assessments.index', $administrator->currentTeam->slug))
            ->assertOk()
        ...
        $response = $this->actingAs($administrator)->post(route('assessments.store', $administrator->currentTeam->slug), [
            'county_id' => $county->id,
            'assessment_cycle_id' => $cycle->id,
        ]);

        $assessment = Assessment::query()->sole();
        $response->assertRedirect(route('assessments.show', [$administrator->currentTeam->slug, $assessment]));
        ...
        $this->actingAs($administrator)->get(route('assessments.show', [$administrator->currentTeam->slug, $assessment]))
            ->assertOk()
            ->assertInertia(fn (AssertableInertia $page) => $page
        ...
        foreach (['csv', 'json'] as $format) {
            $content = $this->actingAs($administrator)
                ->get(route('workspace.export', [$administrator->currentTeam->slug, 'assessments', $format]))
                ->assertOk()
                ->streamedContent();
            $this->assertStringContainsString($release->checksum, $content);
            $this->assertStringContainsString('Assessment Administrator', $content);
        }
        $this->actingAs($administrator)->get(route('workspace.export', [$administrator->currentTeam->slug, 'assessments', 'xlsx']))->assertOk()->assertDownload();
        $this->actingAs($administrator)->get(route('workspace.export', [$administrator->currentTeam->slug, 'assessments', 'pdf']))->assertOk()->assertHeader('content-type', 'application/pdf');
    }
    ...
    public function test_creation_fails_closed_without_a_complete_effective_reference_release(): void
    {
        ...
        $route = route('assessments.store', $administrator->currentTeam->slug);
        ...
        $this->actingAs($administrator)->from(route('assessments.index', $administrator->currentTeam->slug))->post($route, $payload)
    ...
    public function test_creation_rejects_unreleased_scorecards_duplicates_and_unauthorized_counties(): void
    {
        ...
        $this->actingAs($administrator)->post(route('assessments.store', $administrator->currentTeam->slug), [
    ...
    public function test_duplicate_and_unauthorized_http_creation_are_rejected(): void
    {
        ...
        $this->actingAs($official)->post(route('assessments.store', $official->currentTeam->slug), $payload)->assertForbidden();
        $this->actingAs($administrator)->post(route('assessments.store', $administrator->currentTeam->slug), $payload)->assertRedirect();
        $this->actingAs($administrator)->from(route('assessments.index', $administrator->currentTeam->slug))->post(route('assessments.store', $administrator->currentTeam->slug), $payload)
            ->assertSessionHasErrors('county_id');
        $this->assertSame(1, Assessment::query()->count());
    }
