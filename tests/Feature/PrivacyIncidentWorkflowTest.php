...
class PrivacyIncidentWorkflowTest extends TestCase
{
    ...
    public function test_notifiable_breach_requires_independent_assessment_notification_evidence_and_independent_closure(): void
    {
        ...
        $this->actingAs($reporter)->post(route('data-governance.privacy-incidents.store', $reporter->currentTeam->slug), [
        ...
        $this->actingAs($reporter)->patch(route('data-governance.privacy-incidents.advance', [$reporter->currentTeam->slug, $incident]), ['transition' => 'contain', 'containment_actions' => 'Revoked the compromised credential, invalidated sessions and isolated the affected service for forensic review.'])->assertRedirect();
        ...
        $this->actingAs($reporter)->patch(route('data-governance.privacy-incidents.advance', [$reporter->currentTeam->slug, $incident]), $assessment)->assertForbidden();
        $this->actingAs($assessor)->patch(route('data-governance.privacy-incidents.advance', [$assessor->currentTeam->slug, $incident]), $assessment)->assertRedirect();
        ...
        $this->actingAs($reporter)->patch(route('data-governance.privacy-incidents.advance', [$reporter->currentTeam->slug, $incident]), $notification)->assertStatus(409);
        $this->actingAs($reporter)->post(route('data-governance.privacy-incidents.documents.store', [$reporter->currentTeam->slug, $incident]), ['record_purpose' => 'notification', 'title' => 'ODPC and affected-person notification record', 'category' => 'Personal data breach evidence', 'source_type' => 'digital', 'document' => UploadedFile::fake()->create('notification.pdf', 24, 'application/pdf')])->assertRedirect();
        $this->actingAs($reporter)->patch(route('data-governance.privacy-incidents.advance', [$reporter->currentTeam->slug, $incident]), $notification)->assertRedirect();
        ...
        $this->actingAs($assessor)->patch(route('data-governance.privacy-incidents.advance', [$assessor->currentTeam->slug, $incident]), $closure)->assertForbidden();
        $this->actingAs($closer)->patch(route('data-governance.privacy-incidents.advance', [$closer->currentTeam->slug, $incident]), $closure)->assertStatus(409);
        $this->actingAs($reporter)->post(route('data-governance.privacy-incidents.documents.store', [$reporter->currentTeam->slug, $incident]), ['record_purpose' => 'closure', 'title' => 'Verified breach remediation closure pack', 'category' => 'Personal data breach evidence', 'source_type' => 'scanned', 'document' => UploadedFile::fake()->create('closure.pdf', 32, 'application/pdf')])->assertRedirect();
        ...
        $this->actingAs($reporter)->get(route('evidence.preview', [$reporter->currentTeam->slug, $document]))->assertOk();
        $this->actingAs($closer)->patch(route('data-governance.privacy-incidents.advance', [$closer->currentTeam->slug, $incident]), $closure)->assertRedirect();
    ...
    public function test_processor_deadline_non_notifiable_path_escalation_scope_and_exports_are_controlled(): void
    {
        ...
        $this->actingAs($assessor)->patch(route('data-governance.privacy-incidents.advance', [$assessor->currentTeam->slug, $incident]), ['transition' => 'assess', 'severity' => 'low', 'real_risk_of_harm' => 'no', 'risk_assessment' => 'The encrypted backup fragment was unavailable briefly but no unauthorized person acquired or accessed personal data.'])->assertRedirect();
        ...
        $this->actingAs($reporter)->get(route('data-governance.index', $reporter->currentTeam->slug))->assertOk()->assertInertia(fn ($page) => $page->where('privacyIncidents.total', 2));
        foreach (['csv', 'xlsx', 'json', 'pdf'] as $format) {
            $this->actingAs($reporter)->get(route('workspace.export', [$reporter->currentTeam->slug, 'privacy-incidents', $format]))->assertDownload();
        }
    }
}
