...
class CreateSupportTicket
{
    ...
    public function handle(User $requester, array $attributes): SupportTicket
    {
        ...
        abort_if($channel !== 'web' && ! $requester->can(ProgrammePermission::ManageSupportTickets->value), 403, __('support-desk.ticket.errors.assisted_channel_manager_only'));
        ...
        $ticket = DB::transaction(function () use ($requester, $attributes, $countyId, $priority, $channel): SupportTicket {
            ...
            abort_unless($categoryCodes->contains((string) $attributes['category']), 422, __('support-desk.ticket.errors.category_unavailable'));
            abort_unless(in_array($channel, $policy->channels, true), 422, __('support-desk.ticket.errors.channel_unavailable'));
            abort_if($this->servicePolicy->recipients($policy, $countyId, $requestedAt, 1)->isEmpty(), 503, __('support-desk.ticket.errors.no_tier_one_responder'));
        ...
        $this->servicePolicy->recipients($ticket->serviceDeskPolicy, $countyId, $ticket->requested_at, 1)
            ->each(fn (User $responder) => $responder->notify(ProgrammeAlert::translated('support-desk.ticket.notifications.created_title', 'support-desk.ticket.notifications.reference_subject', 'support-desk', messageParameters: ['reference' => $ticket->reference, 'subject' => $ticket->subject])));
