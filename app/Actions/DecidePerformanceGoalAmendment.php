...
class DecidePerformanceGoalAmendment
{
    ...
    public function handle(PerformanceGoalAmendment $amendment, User $decider, array $attributes): PerformanceGoalAmendmentDecision
    {
        ...
        abort_if($amendment->requested_by === $decider->id, 403, 'Requesters cannot decide their own amendment.');
        ...
        $decision = DB::transaction(function () use ($amendment, $decider, $attributes): PerformanceGoalAmendmentDecision {
            ...
            abort_unless($locked->plan->status === 'active', 409, 'This performance plan no longer accepts amendment decisions.');
            abort_if($locked->decision()->exists(), 409, 'This amendment already has a retained decision.');
            abort_unless($locked->goal->versions()->first()?->is($locked->baseVersion), 409, 'The goal definition has changed since this request was submitted.');
            ...
            if ($decisionName === 'approved') {
                ...
                abort_if(abs(($otherWeight + (float) $locked->proposed_snapshot['weight']) - 100) > 0.01, 422, 'The approved amendment must preserve an exact 100 percent plan weight total.');
        ...
        $this->auditLogger->record($decider, $amendment->plan, 'performance.goal.amendment_decided', "Goal amendment request v{$amendment->request_version} {$decision->decision}.", metadata: ['goal_id' => $amendment->performance_goal_id, 'amendment_id' => $amendment->id, 'decision_id' => $decision->id, 'decision_checksum' => $decision->decision_checksum]);
