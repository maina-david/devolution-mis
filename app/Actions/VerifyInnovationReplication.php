    ...
    public function handle(InnovationReplication $replication, User $actor, array $attributes): InnovationReplication
    {
        return DB::transaction(function () use ($replication, $actor, $attributes): InnovationReplication {
            ...
            if (in_array($actor->id, [$replication->created_by, $replication->accountable_user_id, $replication->submitted_by], true)) {
                throw ValidationException::withMessages(['decision' => 'The creator, accountable adopter and outcome submitter cannot verify adoption.']);
            }
            ...
            $this->auditLogger->record($actor, $replication, 'knowledge.innovation_replication.verified', "Replication {$replication->reference} was {$decision}.", $replication->target_county_id, ['decision' => $decision, 'checksum' => $replication->decision_checksum]);
