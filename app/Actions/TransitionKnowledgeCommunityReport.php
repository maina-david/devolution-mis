...
class TransitionKnowledgeCommunityReport
{
    ...
    public function handle(KnowledgeCommunityReport $report, User $actor, string $transition, string $rationale, ?string $resolution, ?string $postAction): KnowledgeCommunityReport
    {
        return DB::transaction(function () use ($report, $actor, $transition, $rationale, $resolution, $postAction): KnowledgeCommunityReport {
            $lockedReport = KnowledgeCommunityReport::query()->with(['workflowInstance', 'reporter.currentTeam', 'post'])->lockForUpdate()->findOrFail($report->id);
            ...
            $teamSlug = $lockedReport->reporter->currentTeam?->slug;
            $lockedReport->reporter->notify(new ProgrammeAlert('Community report updated', "{$lockedReport->reference} is now {$instance->current_state}.", 'knowledge', is_string($teamSlug) ? route('knowledge.index', $teamSlug) : null));
