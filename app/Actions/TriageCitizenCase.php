...
class TriageCitizenCase
{
    ...
    {
        abort_unless($case->status === 'received' && $case->workflow_instance_id === null, 409, __('citizen.triage.errors.received_required'));
        abort_unless($actor->canAccessCounty($case->county), 403, __('citizen.triage.errors.county_scope'));
        ...
        return DB::transaction(function () use ($case, $actor, $attributes): CitizenCase {
            ...
            $this->auditLogger->record($actor, $case, 'citizen_case.triaged', __('citizen.triage.audit.assigned', ['reference' => $case->reference]), $case->county_id, ['assigned_to' => $attributes['assigned_to'], 'priority' => $attributes['priority'], 'reference_data_release_id' => $referenceDataRelease->id, 'reference_data_version' => $referenceDataRelease->version, 'reference_data_checksum' => $referenceDataRelease->checksum]);
