...
class RecordEvaluationFindingActionUpdate
{
    ...
    public function handle(EvaluationFindingAction $action, AssessmentDocument $document, User $actor, float $progress, string $narrative): EvaluationFindingActionUpdate
    {
        ...
        abort_unless($finding->status === 'open' && $action->status !== 'completed', 409, 'This action is no longer open for progress updates.');
        abort_unless($action->accountable_owner_id === $actor->id, 403);
        abort_unless($document->county_id === $finding->county_id && $document->scan_status === 'clean' && $document->record_status === 'active', 409, 'Progress requires clean, active evidence from the same county.');
        abort_unless($document->links()->where('subject_type', $action->getMorphClass())->where('subject_id', $action->id)->exists(), 409, 'Select evidence linked to this action.');
        ...
        $update = DB::transaction(function () use ($action, $document, $actor, $progress, $narrative, $snapshot): EvaluationFindingActionUpdate {
            ...
            abort_if($locked->updates()->where('status', 'pending_verification')->exists(), 409, 'Verify or reject the pending action update first.');
        ...
        $this->auditLogger->record($actor, $update, 'evaluation.finding_action.progress_submitted', "Progress submitted for {$action->code}.", $finding->county_id, ['progress' => $progress, 'document_id' => $document->id, 'checksum' => $update->checksum]);
