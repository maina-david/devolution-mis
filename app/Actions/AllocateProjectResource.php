...
class AllocateProjectResource
{
    ...
    {
        abort_if($project->status === 'closed' || $project->lifecycle_stage === 'closed', 409, __('projects.errors.resource_planning_locked'));
        ...
        return DB::transaction(function () use ($project, $actor, $attributes): ProjectResourceAllocation {
            ...
            abort_if($resource->status !== 'active', 409, __('projects.errors.active_resource_required'));
            ...
            if ($startsOn->lessThan($resource->available_from) || $endsOn->greaterThan($resource->available_to)) {
                throw ValidationException::withMessages(['starts_on' => __('projects.errors.resource_availability_period')]);
            }
            if ($startsOn->lessThan(CarbonImmutable::parse($milestone->planned_start_date)) || $endsOn->greaterThan(CarbonImmutable::parse($milestone->planned_end_date))) {
                throw ValidationException::withMessages(['starts_on' => __('projects.errors.milestone_allocation_period')]);
            }
            ...
            for ($date = $startsOn; $date->lessThanOrEqualTo($endsOn); $date = $date->addDay()) {
                ...
                if ($allocatedRate + $requestedRate > (float) $resource->capacity_per_day + 0.00001) {
                    throw ValidationException::withMessages(['planned_units_per_day' => __('projects.errors.resource_capacity_exceeded', ['date' => $date->toDateString()])]);
                }
            }
            ...
            $this->auditLogger->record($actor, $allocation, 'project.resource_allocated', __('projects.audit.resource_allocated', ['resource' => $resource->code, 'milestone' => $milestone->code]), $project->lead_county_id, [
