...
class CreateEvaluationFinding
{
    ...
    {
        abort_unless($evaluation->status === 'approved', 409, 'Findings can only be issued from an approved evaluation.');
        abort_unless($actor->programmeRole()->hasNationalScope() || ($evaluation->county_id !== null && $actor->canAccessCounty($evaluation->county)), 403);
        abort_if($evaluation->approved_by === $actor->id, 409, 'The evaluation approver cannot issue its follow-up findings.');
        $owner = User::query()->whereKey($attributes['accountable_owner_id'])->firstOrFail();
        abort_unless($evaluation->county_id === null ? $owner->programmeRole()->hasNationalScope() : $owner->canAccessCounty($evaluation->county), 422, 'The accountable owner must be authorized for the evaluation scope.');
        ...
        $this->auditLogger->record($actor, $finding, 'evaluation.finding.created', "Evaluation finding {$finding->reference} issued.", $evaluation->county_id, ['checksum' => $finding->checksum]);
