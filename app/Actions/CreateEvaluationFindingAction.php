...
class CreateEvaluationFindingAction
{
    ...
    {
        abort_unless($finding->status === 'open', 409, 'Actions cannot be added to a closed finding.');
        abort_unless($finding->accountable_owner_id === $actor->id || $actor->programmeRole()->hasNationalScope(), 403);
        abort_if(CarbonImmutable::parse($attributes['due_at'])->startOfDay()->greaterThan($finding->due_at), 422, 'An action cannot be due after the finding deadline.');
        $owner = User::query()->whereKey($attributes['accountable_owner_id'])->firstOrFail();
        abort_unless($finding->county_id !== null && $owner->county_id === $finding->county_id && $owner->access_revoked_at === null, 422, 'The action owner must be an active user in the finding county.');
        ...
        $action = DB::transaction(function () use ($finding, $actor, $attributes, $weight, $snapshot): EvaluationFindingAction {
            ...
            abort_unless($locked->status === 'open', 409, 'Actions cannot be added to a closed finding.');
            $allocated = (float) $locked->actions()->sum('weight_percentage');
            if (($allocated + $weight) > 100) {
                throw ValidationException::withMessages(['weight_percentage' => 'Total action weight cannot exceed 100%.']);
            }
        ...
        $this->auditLogger->record($actor, $action, 'evaluation.finding_action.created', "Action {$action->code} added to {$finding->reference}.", $finding->county_id, ['weight_percentage' => $weight, 'checksum' => $action->checksum]);
