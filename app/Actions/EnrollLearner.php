...
class EnrollLearner
{
    ...
    {
        abort_unless($course->status === 'published', 409, 'Only published courses accept enrolment.');
        if ($course->county_id) {
        ...
        $this->auditLogger->record($learner, $enrollment, 'learning.enrollment.created', "Enrolled in {$course->code}.", $learner->county_id);
