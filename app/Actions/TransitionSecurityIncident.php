...
class TransitionSecurityIncident
{
    ...
    public function handle(SecurityIncident $securityIncident, User $actor, array $attributes): SecurityIncident
    {
        return DB::transaction(function () use ($securityIncident, $actor, $attributes): SecurityIncident {
            ...
            abort_unless(isset($allowed[$transition]) && in_array($incident->status, $allowed[$transition], true), 409, __('security.incident_transition.errors.invalid_state'));
            abort_if($transition === 'acknowledge' && $actor->id !== $incident->incident_lead_id, 403, __('security.incident_transition.errors.assigned_lead_required'));
            if ($transition === 'close') {
            ...
            $this->auditLogger->record($actor, $incident, 'security.incident.'.$transition, __('security.incident_transition.audit', ['reference' => $incident->reference, 'from' => $fromStatus, 'to' => $incident->status]), metadata: ['record_type' => $incident->record_type, 'playbook' => $incident->playbook, 'severity' => $incident->severity]);
    ...
    {
        abort_if(in_array($actor->id, [$incident->reported_by, $incident->incident_lead_id], true), 403, __('security.incident_transition.errors.independent_closer_required'));
        abort_if($incident->data_exposure === 'confirmed' && blank($incident->external_reference), 409, __('security.incident_transition.errors.privacy_reference_required'));
        abort_unless($incident->documentLinks()->where('purpose', 'security-incident-closure-evidence')->whereHas('document', fn ($query) => $query->where('scan_status', 'clean')->where('record_status', 'active'))->exists(), 409, __('security.incident_transition.errors.clean_closure_evidence_required'));
    }
}
