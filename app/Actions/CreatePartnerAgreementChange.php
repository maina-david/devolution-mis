...
class CreatePartnerAgreementChange
{
    ...
    public function handle(PartnerAgreement $agreement, User $requester, array $attributes): PartnerAgreementChangeRequest
    {
        ...
        abort_unless(in_array($agreement->status, ['active', 'suspended'], true), 409, 'Only approved or suspended agreements can enter post-approval change governance.');
        ...
        $request = DB::transaction(function () use ($agreement, $requester, $attributes): PartnerAgreementChangeRequest {
            ...
            abort_if($locked->changeRequests()->whereDoesntHave('decision')->exists(), 409, 'This agreement already has a pending change request.');
