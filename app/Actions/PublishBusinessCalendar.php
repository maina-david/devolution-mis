...
class PublishBusinessCalendar
{
    ...
    public function handle(BusinessCalendar $businessCalendar, User $publisher): BusinessCalendar
    {
        return DB::transaction(function () use ($businessCalendar, $publisher): BusinessCalendar {
            ...
            abort_unless($calendar->status === 'draft', 409, 'Only draft calendars can be published.');
            abort_if($calendar->created_by === $publisher->id, 403, 'Calendar publication requires an actor independent of the author.');
            ...
            abort_if($overlap, 409, 'Published versions of the same calendar cannot have overlapping effective dates.');
            ...
            $this->auditLogger->record($publisher, $calendar, 'workflow.business-calendar.published', "Business calendar {$calendar->code} v{$calendar->version} published.", metadata: ['checksum' => $calendar->checksum]);
