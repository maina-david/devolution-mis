...
class RecordPartnerCollaborationActionUpdate
{
    ...
    public function handle(PartnerCollaborationAction $action, User $submitter, array $attributes): PartnerCollaborationActionUpdate
    {
        ...
        abort_unless($action->plan->status === 'active' && $action->status !== 'completed', 409, 'Only an accountable owner can update an active incomplete action.');
        $progress = (float) $attributes['progress_percentage'];
        abort_if($progress < (float) $action->progress_percentage, 422, 'Progress cannot move backwards.');
        ...
        $update = DB::transaction(function () use ($action, $submitter, $attributes, $progress): PartnerCollaborationActionUpdate {
            ...
            abort_if($locked->updates()->whereDoesntHave('decision')->exists(), 409, 'A previous progress update is awaiting independent verification.');
            ...
            abort_if($progress >= 100 && $documents->isEmpty(), 422, 'Completion requires at least one clean active evidence record.');
            $submittedAt = now();
        ...
        $this->auditLogger->record($submitter, $action, 'partner.collaboration_action.update_submitted', 'Collaboration action progress submitted for independent verification.', $action->county_id, ['update_id' => $update->id, 'update_checksum' => $update->update_checksum]);
