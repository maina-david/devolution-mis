...
class DataGovernanceController extends Controller
{
    ...
    public function storeAsset(StoreDataAssetRequest $request): RedirectResponse
    {
        ...
        $this->auditLogger->record($this->user($request), $asset, 'privacy.data-asset.registered', __('data-governance.privacy.audit.asset_registered', ['code' => $asset->code]));
    ...
    public function storePrivacyIncident(StorePrivacyIncidentRequest $request): RedirectResponse
    {
        ...
        $this->auditLogger->record($user, $incident, 'privacy.incident.reported', __('data-governance.privacy.audit.incident_reported', ['reference' => $incident->reference]), metadata: ['controller_role' => $incident->controller_role, 'regulator_due_at' => $incident->regulator_notification_due_at->toIso8601String()]);
    ...
    private function privacyIncidentPayload(PrivacyIncident $incident, bool $canManage): array
    {
        return [
            ...
            'description' => $canManage ? $incident->description : __('data-governance.privacy.labels.restricted_incident_narrative'), 'personalDataCategories' => $incident->personal_data_categories, 'estimatedDataSubjects' => $incident->estimated_data_subjects, 'containsSensitiveData' => $incident->contains_sensitive_data,
