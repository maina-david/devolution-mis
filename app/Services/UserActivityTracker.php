...
class UserActivityTracker
{
    ...
    {
        $activitySession = UserActivitySession::query()->firstOrCreate(['session_fingerprint' => $this->fingerprint($request)], ['user_id' => $user->id, 'ip_address' => $request->ip(), 'user_agent' => $request->userAgent(), 'logged_in_at' => now(), 'last_seen_at' => now()]);
    ...
    public function touch(Request $request, User $user, string $action, ?string $pageTitle = null): UserActivitySession
    {
        ...
        $activitySession->update(['current_route' => $request->route()?->getName(), 'current_path' => '/'.ltrim($request->path(), '/'), 'current_page_title' => $pageTitle, 'last_method' => $request->method(), 'last_action' => $action, 'last_seen_at' => now(), 'logged_out_at' => null]);
