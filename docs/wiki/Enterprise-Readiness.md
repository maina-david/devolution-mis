...
The localization extraction programme now covers the authenticated navigation registry, settings navigation, reusable data-table controls, searchable selects, table empty states and account actions in English, Kiswahili and French. Knowledge, learning, security-governance, IGR, integration and data-governance mutation outcomes, plus controlled CSV/XLSX import validation, are also extracted. The learning overview, catalogue/cohort/offline-sync registers, cohort lifecycle and offline reconciliation surfaces now use synchronized catalogues, locale-aware dates and localized assistive labels. A source audit records 1,793 remaining frontend literals across 104 files and 654 backend candidates and fails when either corpus increases; this is a regression control, not a claim that extraction is complete.
