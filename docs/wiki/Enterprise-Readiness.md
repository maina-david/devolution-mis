...
The localization extraction programme now covers the authenticated navigation registry, settings navigation, reusable data-table controls, searchable selects, table empty states and account actions in English, Kiswahili and French. Knowledge, learning, security-governance, IGR, integration, operations, evaluation findings and data-governance mutation outcomes, plus controlled CSV/XLSX import validation, are also extracted. The complete learning, analytics, security-governance, reference-data, project-record, knowledge-management, support-desk, governed assessment-record, IGR, operations, departmental-performance, integration-management and workflow-management workspaces plus the shared evaluation-finding and monitoring-results surfaces use synchronized catalogues and have zero governed JSX literals. A source audit records 949 remaining frontend literals across 89 files and 654 backend candidates and fails when either corpus increases; this is a regression control, not a claim that application-wide extraction is complete.
