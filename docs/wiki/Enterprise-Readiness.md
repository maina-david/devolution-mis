...
The localization extraction programme now covers the authenticated navigation registry, settings navigation, reusable data-table controls, searchable selects, table empty states and account actions in English, Kiswahili and French. A source audit records the remaining hard-coded corpus and fails when frontend or backend localization debt increases; this is a regression control, not a claim that extraction is complete.
