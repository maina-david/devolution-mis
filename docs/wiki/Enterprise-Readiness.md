...
The localization extraction programme now covers the authenticated navigation registry, settings navigation, reusable data-table controls, searchable selects, table empty states and account actions in English, Kiswahili and French. Knowledge, learning, security-governance, IGR, integration and data-governance mutation outcomes, plus controlled CSV/XLSX import validation, are also extracted. A source audit records the remaining hard-coded corpus and fails when frontend or backend localization debt increases; this is a regression control, not a claim that extraction is complete.
